...
#pragma mark - Tiny HUD (optional feedback)

static UIView *QTShowHUD(NSString *text) {
    UIWindow *w = QTGetKeyWindow();
    if (!w) return nil;

    // remove old hud
    UIView *old = [w viewWithTag:998877];
    ...
    lbl.backgroundColor = [[UIColor blackColor] colorWithAlphaComponent:0.70];
    ...
    CGFloat padX = 16;
    CGFloat padY = 10;
    CGSize max = CGSizeMake(MIN(320, w.bounds.size.width - 40), CGFLOAT_MAX);
    CGRect r = [lbl.text boundingRectWithSize:max
                                      options:NSStringDrawingUsesLineFragmentOrigin
                                   attributes:@{NSFontAttributeName: lbl.font ?: [UIFont systemFontOfSize:14]}
                                      context:nil];
    CGFloat width = ceil(r.size.width) + padX * 2;
    CGFloat height = ceil(r.size.height) + padY * 2;

    lbl.frame = CGRectMake((w.bounds.size.width - width)/2.0,
                           w.safeAreaInsets.top + 18,
                           width,
                           height);
...
static void QTHideHUD(void) {
    UIWindow *w = QTGetKeyWindow();
    if (!w) return;
    UIView *hud = [w viewWithTag:998877];
...
static void QTLibreTranslate(NSString *serverURL,
    ...
    [[session dataTaskWithRequest:req completionHandler:^(NSData * _Nullable data,
...
static void QTTranslateVisibleScreen(void) {
    ...
    QTRunOCR(snap, ^(NSString *text, NSError *err) {
        ...
        // Limit characters (performance + service limits)
...
static void QTInstallFloatingButton(void) {
    ...
    // remove button if disabled/whitelist off
